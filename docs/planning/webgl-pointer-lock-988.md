...
2026-09-14 사용자 실제 Chrome 확인: v7 브라우저 코드 검증본에서 창 모드의 설정 복귀 후 자동 이동 문제가 해결됐다. 사용자는 창 모드의 마우스 재클릭은 이번 작업에서 더 처리하지 않고 넘어가기로 결정했다. 해당 항목은 해결 완료가 아닌 사용자 결정에 따른 보류다.

수정 소스 커밋은 1a90d868이다. 사용자 Chrome 입력 확인은 v6 wasm에 브라우저 연결 함수를 적용한 v7 검증본에서 이뤄졌다. 이후 같은 수정이 포함된 정식 Unity WebGL·Linux 빌드 `988-rollout-v2`를 완성하고 새 WebGL의 실제 방 생성·퇴장·재생성을 확인했다. 현재 테스트 호스트에서 제공하며 `.tar`로 보관했다. 새 빌드의 Chrome 입력 재확인이나 6인 경기 전체 통과를 뜻하지는 않는다. [빌드·버전 교체 결과](server-release-host-988.md)를 참고한다. MR/push는 하지 않았고, 별도 PC의 WebGL 6인 경기·presence 401·서버 수용량 검증은 별도로 남아 있다.
...
실제 검증을 빠르게 하기 위해 v6 실행 파일의 GamePointerArm/Release를 현재 저장소 jslib 함수로 교체한 v7 브라우저 코드 검증본을 별도 폴더에 준비했다. .build/server-988-webgl-input-v7-preview이며 전체 Unity 재빌드를 거친 최종 패키지는 아니다. 기존 wasm/C#은 유지하고 전체 framework 문법 검사 통과. 기존 v6 출력과 ZIP은 새 후보로 대체하지 않았다. 이후 사용자 실제 Chrome에서 창 모드 자동 이동 해결을 확인했다. 정식 Unity 재빌드와 최종 패키지는 남아 있다.

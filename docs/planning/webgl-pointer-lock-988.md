...
수정 소스 커밋은 1a90d868이다. 실제 확인한 것은 v6 wasm에 현재 소스의 브라우저 연결 함수를 적용한 v7 검증본이며, 저장소 소스로 만든 정식 Unity WebGL 재빌드와 최종 배포용 ZIP은 아직 남아 있다. MR/push는 하지 않았다. 서로 다른 PC의 WebGL 6인 경기·presence 401·서버 수용량 검증은 별도 미완료 항목이다.

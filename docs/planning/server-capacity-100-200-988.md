...
Photon 공식 기본 권장은 WebGL에서 Shared Mode다. Client/Server는 별도 설정으로 활성화할 수 있지만, Shared보다 클라이언트 재시뮬레이션 CPU와 연결 지연 부담이 크다. 실제 SDK도 현재 WebGL Client 실행에 경고를 남긴다. 따라서 여기서 제안한 Server Mode는 **현재 게임의 서버 권한을 유지하기 위한 조건부 선택**이며, WebGL에 가장 저렴하거나 공식 권장인 선택이라는 뜻이 아니다. [Photon 플랫폼별 선택 지침](https://doc.photonengine.com/fusion/v2/fusion-choose#webgl-builds)

외부 Linux에서 서버를 돌리므로 브라우저 탭 비활성화가 방 전체 서버를 멈추는 문제는 피하지만, WebGL 클라이언트 CPU·입력·네트워크 제한은 남는다. 저사양을 포함한 서로 다른 PC의 6인 WebGL 경기에서 프레임·입력·지연·탭 전환 복귀를 먼저 통과시킨 뒤 100~200명 확장 실험으로 넘어간다. 장비 증설은 클라이언트 병목을 해결하지 않는다.

이 조건을 충족하지 못하면 GameLift 계약이나 Multi-Peer 전환보다 앞서 Shared Mode의 소규모 기능 비교를 수행한다. Shared는 서버 게임 연산 비용을 줄일 수 있으나 플레이어의 상태 권한이 달라져 판정·치트 방지·경기 결과 검증 설계를 바꿔야 한다. 단순히 현재 구현을 보존한다는 이유만으로 운영 구조를 확정하지 않는다.

...
플레이어 Host를 대체할 기술 후보로 **네이티브 Unity 프로세스의 Fusion `GameMode.Server` + WebGL `GameMode.Client`**를 선택한다. 경기 권한과 비공개 배정, 물리 확정, 하이라이트 기록은 서버가 소유한다. 방장은 플레이어 중 로비 관리 권한을 가진 사람으로 분리한다. 이는 별도 Unity 프로세스를 직접 운영한다는 전제이며, Photon이 게임 실행 파일을 호스팅해 주는 방식이 아니다. 기존 서버의 수용량과 운영 비용은 아직 확인하지 않았으므로 배포 환경까지 확정한 결정은 아니다.
...
최초 WebGL 실행은 localhost에 대한 API CORS 허용 헤더가 없어 인증 단계에서 중단됐다. 동일 출처 계정 전달 경로를 마련하고 사용자 명시적 승인 후 재실행했다. 기존 인증·TLS·운영 CORS 설정은 그대로 유지했다.

**실제 WebGL 최소 접속 실험은 통과했다.** 같은 PC의 브라우저 두 탭이 별도 네이티브 Server에 Photon Cloud를 통해 연결되어 상태 복제·RPC 처리·송신자별 Reliable Data 회신을 확인했다. 한 Client 종료 후 다른 Client의 tick이 계속 진행됐고, 종료한 Client의 재접속도 성공했다. 두 Client의 정상 종료, 다른 버전과 없는 방의 `GameNotFound` 거절을 확인했다. [상태 로그](../../Tools/network/topology-probe/evidence-2026-09-14.txt)와 재실행 가능한 판정 도구를 함께 보관한다.

이 결과는 최소 네트워크 실험이며, 서로 다른 PC의 6인 실제 게임·전체 로비/경기/하이라이트/재경기·서버 장애 복구·운영 수용량·120FPS를 검증한 결과는 아니다. 제품은 여전히 Host 모드이고 실제 전환은 988 범위다.

...
- 수정:
  1. 끌어당김 제거. 손 닿는 거리(가슴 높이 1.2 m 기준, `InteractionConfig.placementMaxDistance` 1 → **1.3 m**)를 넘으면 그 자리에 **빨간 고스트 + "너무 멀어요"** 표시. 광선은 거리보다 0.6 m 더 멀리 봐서 초과 자리도 보여 준다.
  2. 수직면(법선 |y| < 0.5)을 조준하면 광선 방향으로 0.15 m 안쪽에서 위 0.35 m / 아래 0.6 m 범위로 윗면을 찾아 그 위에 놓는다(선반 판·상품 위).
